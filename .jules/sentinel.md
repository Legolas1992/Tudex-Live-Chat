...
## 2024-06-20 - [Fix Object Type Confusion in Auth Endpoints]
**Vulnerability:** Registration and profile endpoints implicitly cast unvalidated request payloads (`req.body.username`, `req.body.password`) to strings using `String()`. By passing an object (e.g. `{"$ne": null}`) as the password during profile update, it bypasses length checks (since `String(obj)` is `"[object Object]"`) and sets the user's password hash to a predictable string.
**Learning:** Type casting with `String()` in Node.js does not validate types. Objects are stringified as `"[object Object]"`, which can bypass length validations and lead to logic flaws or predictable credentials.
**Prevention:** Always explicitly type-check (e.g., `typeof input !== 'string'`) and throw errors or return 400 Bad Request for invalid types before casting or validating.
