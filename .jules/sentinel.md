...
## 2024-06-18 - [Fix CPU Exhaustion DoS in Synchronous Hashing]
**Vulnerability:** The `hashPassword` and `verifyPassword` functions used `crypto.pbkdf2Sync` to hash user-provided passwords. This native Node.js function blocks the event loop synchronously. Because there was no length limit on the input password, an attacker could supply an extremely long string (e.g., millions of characters), causing the hashing function to take a very long time. This would block the Node.js event loop, preventing all other requests from being processed and causing a Denial of Service (DoS) attack.
**Learning:** Synchronous cryptographic operations block the Node.js event loop. If the input size is unbounded, these operations can be leveraged for a CPU exhaustion Denial of Service (DoS).
**Prevention:** Always enforce strict length limits on user inputs, especially before passing them to expensive, synchronous operations like `pbkdf2Sync`. A limit of 72 characters is a standard maximum length for passwords, aligning with bcrypt's limitations.
