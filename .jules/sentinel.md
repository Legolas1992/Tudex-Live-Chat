...
## 2024-07-04 - [Fix Stored XSS in File Upload]
**Vulnerability:** The `/api/upload` endpoint used `path.extname(fileName)` directly from user input to determine the file extension of the saved file without any validation. Because the uploaded files were then served using `express.static`, an attacker could upload an HTML file with malicious JavaScript (e.g. `fileName: 'test.html'`). The static server would serve it with `Content-Type: text/html`, resulting in a Stored Cross-Site Scripting (XSS) vulnerability.
**Learning:** File upload endpoints must never trust the user-provided file extension or MIME type. If files are served statically, any unsafe extension (like `.html`, `.svg`, `.xml`) can execute scripts in the context of the application's domain when accessed by another user.
**Prevention:** Always sanitize and validate file extensions against a strict allowlist of safe media types (e.g., `.png`, `.jpg`, `.mp4`). Fallback to a safe extension like `.bin` if the extension is unknown or unsafe, ensuring the browser treats the file as a generic binary rather than an executable format.
