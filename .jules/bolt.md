...
## 2024-06-18 - [Mongoose Sort Optimization]
**Learning:** Adding unindexed tie-breaker fields (e.g., `createdAt: -1`) to a query sorted by a partially matched index (e.g., `timestamp: -1`) triggers a slow, memory-intensive in-memory sort.
**Action:** When optimizing Mongoose queries, ensure that `.sort()` parameters align perfectly with the trailing fields of the chosen compound index.
